    ...
    private void configureBindings() {
        drivetrain.setDefaultCommand(
                drivetrain.applyRequest(() -> drive
                        .withVelocityX(
                                -m_leftJoystick.getY() * MaxSpeed * (slowModeOn ? DriveConstants.SLOW_MODE_MULT : 1))
                        .withVelocityY(
                                -m_leftJoystick.getX() * MaxSpeed * (slowModeOn ? DriveConstants.SLOW_MODE_MULT : 1))
                        .withRotationalRate(
                                -m_rightJoystick.getX() * MaxAngularRate
                                        * (slowModeOn ? DriveConstants.SLOW_MODE_MULT : 1))));
